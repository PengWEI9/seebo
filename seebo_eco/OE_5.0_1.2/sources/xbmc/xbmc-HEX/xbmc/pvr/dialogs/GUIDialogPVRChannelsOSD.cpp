...
void CGUIDialogPVRChannelsOSD::Update()
{
  ...
  m_viewControl.SetCurrentView(DEFAULT_VIEW_MAX);
  ...
  if (group)
  {
    ...
    if(!m_group)
    {
      ...
      //TBIRD - ECO - START
      CLog::Log(LOGNOTICE, "--------- Channel Index: %d", group->GetIndex(*channel)); 
      CLog::Log(LOGNOTICE, "--------- Channel Group ID: %d", group->GroupID());   
      //TBIRD - ECO - FINISH
...
void CGUIDialogPVRChannelsOSD::SaveSelectedItem(int iGroupID)
{
  m_groupSelectedItems[iGroupID] = m_viewControl.GetSelectedItem();
   ...
   //TBIRD - ECO - START 
   CLog::Log(LOGNOTICE, "--------- iGroupID: %d", iGroupID);   
   //TBIRD - ECO - FINISH
}
