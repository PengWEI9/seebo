...
//#include "settings/lib/ISettingCallback.h" //HPrasad

namespace PVR
{
  class CGUIWindowPVR;
  // HPrasad || Added the support of Settings Callback
  ...
  public:    
    ...
    static void SettingOptionsIPTVList(const CSetting *setting, std::vector< std::pair<std::string, std::string> > &list, std::string &current); //HPrasad
    static std::string getEPG(unsigned int); // HPrasad
    static std::string getM3U(unsigned int); // HPrasad
    static int getLanNode(std::string);
    static void updateIPTVSetting(/*unsigned int ,*/std::string ,std::string); // HPrasad
  ...
  private:
    ...
    bool OnContextButtonShowEpg(CFileItem *item, CONTEXT_BUTTON button);
