...
//Jin - ECO - START(-/+)
    if(!strcmp(status, "Channel: no data")){
        XBMC->QueueNotification(SEEBO_INFO, status);    
        system("/storage/.xbmc/addons/service.multimedia.vdr-addon/bin/./vdr.stop");
    }
    else XBMC->QueueNotification(QUEUE_INFO, status);
//Jin - ECO - FINSH
  }
  delete[] status;
}
