...
{
  XBMC->Log(LOG_ERROR, " -......................................................this is in VNSIData AddTimer");
  cRequestPacket vrp;
  ...
  // XBMC->Log(LOG_ERROR, " -....................................................................................state             %d",timerinfo.state);
  // XBMC->Log(LOG_ERROR, " -....................................................................................clientIndex       %d",timerinfo.iClientIndex);
  // XBMC->Log(LOG_ERROR, " -....................................................................................iClientChannelUid %d",timerinfo.iClientChannelUid);
  // XBMC->Log(LOG_ERROR, " -....................................................................................startTime         %ld",starttime);
  // XBMC->Log(LOG_ERROR, " -....................................................................................endTime           %ld",endtime);
  ...
  uint32_t returnCode = vresp->extract_U32();
  delete vresp;
  XBMC->Log(LOG_ERROR, " -..............................returned code from add timer: %d", returnCode);
  if (returnCode == VNSI_RET_DATALOCKED)
  {
    // XBMC->Log(LOG_ERROR, " - ***************IN VNSIDATA******ADDTIMER**************IF ITS OVER CONTAINED IN CONTAINED**************  %d",this->DeleteTimer(timerinfo, true));
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-iClientIndex: %d",timerinfo.iClientIndex);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-startTime: %ld",(long)timerinfo.startTime);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-iMarginStart: %d",timerinfo.iMarginStart);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-strDirectory: %s",timerinfo.strDirectory);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-bIsRepeating: %d",timerinfo.bIsRepeating);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-iEpgUid: %d",timerinfo.iEpgUid);
    // PVR->TriggerTimerUpdate();
    // PVR->TriggerRecordingUpdate();
...
{
  XBMC->Log(LOG_ERROR, " - ***********************************DELETE TIMER FROM PVR ADDON -- state is **************  %d", timerinfo.state);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-iClientIndex: %d",timerinfo.iClientIndex);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-startTime: %ld",(long)timerinfo.startTime);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-startTime: %d",timerinfo.iMarginStart);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-strDirectory: %s",timerinfo.strDirectory);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-bIsRepeating: %d",timerinfo.bIsRepeating);
  XBMC->Log(LOG_ERROR, " -&&&&&&&&&&&&&&&&&&&&&&&&&&&TimerInfo-iEpgUid: %d",timerinfo.iEpgUid);
  cRequestPacket vrp;
  ...
  XBMC->Log(LOG_ERROR, " -..............................returned code from TIMER DELETE: %d", returnCode);
...
//peng
bool cVNSIData::isMultiplexOccupied(int clientChannelUid, int state, int clientIndex, int startTime)
{
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    1");
  cRequestPacket vrp;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    2");
  if (!vrp.init(VNSI_TIMER_ADD))
  {
    XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    3");
    XBMC->Log(LOG_ERROR, "%s - Can't init DIRECT cRequestPacket", __FUNCTION__);
    return PVR_ERROR_UNKNOWN;
  }
  ...
  // vrp.add_U32(PVR_TIMER_STATE_SCHEDULED);
  // // vrp.add_U32(timerinfo.iPriority))   return PVR_ERROR_UNKNOWN;
  // // vrp.add_U32(timerinfo.iLifetime))   return PVR_ERROR_UNKNOWN;
  // vrp.add_U32(channel->m_iUniqueId);
  // vrp.add_U32(0);

  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    4");
  if (!vrp.add_U32(state))     return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    5");
  if (!vrp.add_U32(0))   return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    6");
  if (!vrp.add_U32(0))   return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "~isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    7");
  if (!vrp.add_U32(clientChannelUid)) return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    8");
  if (!vrp.add_U32(startTime))  return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "~isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    9");
  if (!vrp.add_U32(0))    return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    10");
  if (!vrp.add_U32(0))   return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    11");
  if (!vrp.add_U32(0)) return PVR_ERROR_UNKNOWN;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    12");
  // if (!vrp.add_String(path.c_str()))      return PVR_ERROR_UNKNOWN;
  // if (!vrp.add_String(""))                return PVR_ERROR_UNKNOWN;

  cResponsePacket* vresp = ReadResult(&vrp);
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    13");
  if (vresp == NULL || vresp->noResponse())
  {
    XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    14");
    delete vresp;
    XBMC->Log(LOG_ERROR, "%s - Can't get response packed", __FUNCTION__);
    return PVR_ERROR_UNKNOWN;
  }
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    15");
  uint32_t returnCode = vresp->extract_U32();
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    16");
  delete vresp;
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    17");
  XBMC->Log(LOG_ERROR, " -..............................Direct returned code from add timer: %d", returnCode);
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    18");
  if (returnCode == VNSI_RET_DATALOCKED)
  {
    XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    19");
    // XBMC->Log(LOG_ERROR, " - ***************IN VNSIDATA******ADDTIMER**************IF ITS OVER CONTAINED IN CONTAINED**************  %d",this->DeleteTimer(timerinfo, true));
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-iClientIndex: %d",timerinfo.iClientIndex);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-startTime: %ld",(long)timerinfo.startTime);
    // XBMC->Log(LOG_ERROR, " - *********************************************************************TimerInfo-iMarginStart: %d",timerinfo.iMarginStart);
  ...
  XBMC->Log(LOG_ERROR, "isMultipleOccupied~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp    20");
  return false;
}

bool cVNSIData::isMultiplexOccupied_(int clientChannelUid, int state, int clientIndex, int startTime)
{

  XBMC->Log(LOG_ERROR, "isMultipleOccupied_________________________~~~~~~~~~~~~~~~~~~~~~~~~~~~~This is in VNSIDate.cpp   00000000 ");
  return false;
}
