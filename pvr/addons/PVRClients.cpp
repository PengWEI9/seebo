...
PVR_ERROR CPVRClients::GetChannels(CPVRChannelGroupInternal *group)
{
  ...
  for (PVR_CLIENTMAP_CITR itrClients = clients.begin(); itrClients != clients.end(); itrClients++)
  {
        ...
        currentError != PVR_ERROR_NO_ERROR)
    {
      ...
      //TBIRD - KOWAN - START
      //Contruct the message and notify the error to the user:
      ErrorMessage = "Cannot get channels for channel; check logs for more details...";
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Error, g_localizeStrings.Get(19166), ErrorMessage);
      //TBIRD - KOWAN - FINISH
    }
  }
...
bool CPVRClients::UpdateAndInitialiseClients(bool bInitialiseAllClients /* = false */)
{
  ...
  m_bIsClientsInitialized = true;
  return bReturn;
}

bool CPVRClients::HasConnectedAllClients(void) const
{
  CSingleLock lock(m_critSection);
  ...
  for (PVR_CLIENTMAP_CITR itr = m_clientMap.begin(); itr != m_clientMap.end(); itr++)
  {
    if (itr->second->ReadyToUse() == false)
    {
      lock.Leave();
      return false;
    }
