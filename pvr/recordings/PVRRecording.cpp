...
bool CPVRRecording::DeleteQuiet(void)
{
  PVR_ERROR error = g_PVRClients->DeleteRecording(*this);
  if (error != PVR_ERROR_NO_ERROR)
    return false;
...
void CPVRRecording::DisplayError(PVR_ERROR err) const
{
  ...
  //CGUIDialogOK::ShowAndGetInput(19033,19147,19110,0); /* print info dialog "Unknown error!" */
	...
	//Tuan's mods 1.3.5 - Improve User Dialogues
	if (err == PVR_ERROR_SERVER_ERROR)
		CGUIDialogOK::ShowAndGetInput(38100,38105,38102,0); /* print info dialog "Server error!" */
	else if (err == PVR_ERROR_REJECTED)
		CGUIDialogOK::ShowAndGetInput(38100,38105,38102,0); /* print info dialog "Couldn't delete recording!" */
	else
		CGUIDialogOK::ShowAndGetInput(38100,38105,38102,0); /* print info dialog "Unknown error!" */
	//Tuan's mods finish
  return;
}
